...
### 多源知识库支持 (2026-02-11 21:45) ✅

**新增功能**：支持多个文本源作为知识库

**支持的源**：
1. **小说原文** (`雪中悍刀行.txt`) - 完整的小说文本
2. **解说全集** (`雪中悍刀行_解说全集.txt`) - 故事时间线、人物解析、剧情梳理

**实现方案**：

| 组件 | 功能 | 文件 |
|------|------|------|
| `MultiSourceIndexer` | 多源统一索引器 | `src/multi_source_indexer.py` |
| `build_multi_source.py` | 多源知识库构建脚本 | `build_multi_source.py` |
...
# 或直接使用多源构建脚本
python build_multi_source.py
```

**生成文件**：
- `chunks.json` - 所有文本块（包含来源标记）
- `sources.json` - 来源信息
- `keyword_index.json` - 关键词索引
- `commentary_summaries.json` - 解说全集章节摘要
- `commentary_knowledge.json` - 解说全集专家知识库
